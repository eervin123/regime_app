...
def forecast_next_regime(daily_data, ticker, hypothetical_change=0.01):
    ...
    new_row = pd.DataFrame({ticker: [hypothetical_price], 'Return': [np.nan]}, index=[daily_data.index[-1] + pd.Timedelta(days=1)])
...
for i, ticker in enumerate(daily_data.keys()):
    with col1 if i == 0 else col2:
        changes[ticker] = st.number_input(f"{ticker} Expected Return (%)", value=0.0, step=0.1)

if st.button("Forecast Regimes"):
    st.subheader("Projected Regimes for Tomorrow")
    col1, col2 = st.columns(2)
    for i, (ticker, data) in enumerate(daily_data.items()):
        projected_regime = forecast_next_regime(data, ticker, hypothetical_change=changes[ticker]/100)
        with col1 if i == 0 else col2:
